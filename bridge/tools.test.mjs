...
test('read_file repeat guard avoids returning an unchanged full file twice', async () => {
  ...
  const firstOutput = await invokeTool(
    readFile,
    {
      path: 'notes.md',
    },
  ...
  const secondOutput = await invokeTool(
    readFile,
    {
      path: 'notes.md',
    },
...
test('builtin run_shell records workspace files generated by commands', async () => {
  const workspace = await fs.mkdtemp(path.join(os.tmpdir(), 'aura-run-shell-mutation-'))
  await fs.writeFile(path.join(workspace, 'source.md'), '# Title\n\nBody\n')
  const runShellTool = createBuiltinTools({ cwd: workspace }).find(tool => tool.name === 'run_shell')
  const events = []
  ...
  const output = await invokeTool(
    runShellTool,
    {
      command: 'cp source.md generated.md',
    },
    events,
    {
      settings: {
  ...
  assert.equal(parsed.status, 'exited')
  assert.equal(parsed.exitCode, 0)
  assert.deepEqual(parsed.affectedPaths, ['generated.md'])
  assert.equal(parsed.files[0].kind, 'create')
  assert.equal(parsed.files[0].diffStat.addedLines, 3)
  assert.equal(events[0].structuredOutput.operation, 'shell_file_mutation')
  assert.equal(events[0].structuredOutput.files[0].path, 'generated.md')
})
