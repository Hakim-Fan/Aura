...
function BashPermissionRequestInner({
  ...
  // sufficient — no latch/ref needed. The  ternary keeps the property
  // read out of external builds (forbidden-string check).
  const [classifierWasChecking] = useState( ? !!toolUseConfirm.classifierCheckInProgress : false);
  ...
  useKeybinding('confirm:no', handleDismissCheckmark, {
    context: 'Confirmation',
    isActive:  ? !!toolUseConfirm.classifierAutoApproved : false
  });
  function onSelect(value_0: string) {
    ...
    if ( && value_0 === 'yes-classifier-reviewed') {
      ...
      {showPermissionDebug ? <>
          ...
          <Box flexDirection="column">
            ...
            {destructiveWarning_0 && <Box marginBottom={1}>
                <Text color="warning" dimColor={ ? toolUseConfirm.classifierAutoApproved : false}>
                  {destructiveWarning_0}
                </Text>
              </Box>}
            <Text dimColor={ ? toolUseConfirm.classifierAutoApproved : false}>
              Do you want to proceed?
            </Text>
            <Select options={ ? toolUseConfirm.classifierAutoApproved ? options.map(o => ({
          ...o,
          disabled: true
        })) : options : options} isDisabled={ ? toolUseConfirm.classifierAutoApproved : false} inlineDescriptions onChange={onSelect} onCancel={() => handleReject()} onFocus={handleFocus} onInputModeToggle={handleInputModeToggle} />
          </Box>
