...
const VoiceKeybindingHandler: typeof import('../hooks/useVoiceIntegration.js').VoiceKeybindingHandler =  ? require('../hooks/useVoiceIntegration.js').VoiceKeybindingHandler : () => null;
...
const proactiveModule =  ||  ? require('../proactive/index.js') : null;
const PROACTIVE_NO_OP_SUBSCRIBE = (_cb: () => void) => () => {};
const PROACTIVE_FALSE = () => false;
const SUGGEST_BG_PR_NOOP = (_p: string, _n: string): boolean => false;
const useProactive =  ||  ? require('../proactive/useProactive.js').useProactive : null;
const useScheduledTasks =  ? require('../hooks/useScheduledTasks.js').useScheduledTasks : null;
...
/* eslint-disable @typescript-eslint/no-require-imports */
const WebBrowserPanelModule =  ? require('../tools/WebBrowserTool/WebBrowserPanel.js') as typeof import('../tools/WebBrowserTool/WebBrowserPanel.js') : null;
/* eslint-enable @typescript-eslint/no-require-imports */
...
export function REPL({
  ...
  const disableMessageActions =  ?
  // biome-ignore lint/correctness/useHookAtTopLevel:  is a compile-time constant
  ...
  //  is a build-time constant — dead code elimination removes the hook
  ...
  // KAIROS build + config.viewerOnly.  is build-time constant so
  ...
  const getToolUseContext = useCallback((messages: MessageType[], newMessages: MessageType[], abortController: AbortController, mainLoopModel: string): ProcessUserInputContext => {
    ...
    return {
      ...
      resume,
      setConversationId,
      requestPrompt:  ? requestPrompt : undefined,
  ...
  const onQueryImpl = useCallback(async (messagesIncludingNewMessages: MessageType[], newMessages: MessageType[], abortController: AbortController, shouldQuery: boolean, additionalAllowedTools: string[], mainLoopModelParam: string, effort?: EffortValue) => {
    ...
    // Gated on TRANSCRIPT_CLASSIFIER so GrowthBook kill switch runs wherever auto mode is built in
     ? checkAndDisableAutoModeIfNeeded(toolPermissionContext, setAppState, store.getState().fastMode) : undefined, getSystemPrompt(freshTools, mainLoopModelParam, Array.from(toolPermissionContext.additionalWorkingDirectories.keys()), freshMcpClients), getUserContext(), getSystemContext()]);
    const userContext = {
      ...baseUserContext,
  ...
  if (screen === 'transcript') {
    ...
    const transcriptReturn = <KeybindingSetup>
        ...
        { ? <VoiceKeybindingHandler voiceHandleKeyEvent={voice.handleKeyEvent} stripTrailing={voice.stripTrailing} resetAnchor={voice.resetAnchor} isActive={!toolJSX?.isLocalJSXCommand} /> : null}
      ...
      <MCPConnectionManager key={remountKey} dynamicMcpConfig={dynamicMcpConfig} isStrictMcpConfig={strictMcpConfig}>
        ...
        jumpToNew(scrollRef.current);
          ...
          type: 'text'
              ...
              { ? WebBrowserPanelModule && <WebBrowserPanelModule.WebBrowserPanel /> : null}
              <Box flexGrow={1} />
