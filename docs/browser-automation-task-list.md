...
1. Phase 1-7 已完成，包括浏览器设置页、运行时检测、Aura 浏览器运行时、托管浏览器安装卸载、可见接管、Chrome 站点级登录态导入与已导入站点管理。
2. 已导入站点当前已支持刷新导入、清理 Aura 状态、删除记录，且“清理 Aura 状态”会真实删除 Aura Profile 内该站点 Cookie。
3. 聊天侧当前已接入 blocker 卡片，并可向同一条任务流追加“打开浏览器接管 / 继续执行”指令。
4. Aura 浏览器 Profile 当前已支持打开目录、重置全部站点会话、清空整份 Profile。

本轮补充完成：

1. 为系统 Chrome 备用模式补上“运行时不可用时允许自动降级”开关。
2. 清理通用审批区里的旧 `Chrome 自动化默认自动允许` 入口，将其收拢到浏览器页的“系统 Chrome 备用模式”模块。
3. 强化模型侧路由指引：默认网页任务保持走 `browser_*`，只有用户明确要求或 Aura 浏览器运行时不可用且允许降级时才走 `chrome_*`。
4. 补上 Aura Profile 的“重置全部站点会话 / 清空整份 Profile”操作，并完善已导入站点的来源与时间展示。
5. 回填设计文档与本清单的实现进度、验证结果和收尾说明。
6. 移除设置页中浏览器状态的占位展示，改为真实运行策略说明。
...
4. blocker 识别 smoke test 返回 `blocker.detected: true`，并带明确 `reason`
...
1. “导入 cookie -> 页面生效 -> 清理 Aura 状态 -> 页面失效”闭环已跑通，结果为 `before=true`、`after=false`
2. 聊天侧已能展示 takeover 卡片，并可把“打开浏览器接管 / 继续执行”转成同任务流里的追加指令
3. 默认网页任务的文案、设置入口与模型路由说明已统一收口到 Aura 浏览器运行时
...
1. 设计文档已补充当前实现进度与验证记录
2. 浏览器设置页中与“默认前台 Chrome 路径”相关的旧表述已收口
3. Aura Profile 的整份清空与全量站点会话重置现已接入真实操作，并带确认提示
