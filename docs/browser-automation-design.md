...
1. 默认网页任务改走 Aura 浏览器运行时，并使用 Aura 自己的持久化 Profile。
2. 浏览器设置页、运行时检测、托管浏览器安装卸载、自定义可执行文件、搜索偏好和浏览器行为偏好都已落地。
3. blocker 检测、`browser_takeover_visible`、`browser_resume_after_takeover` 以及聊天侧 takeover 卡片与任务内联动都已接通。
4. Chrome 登录缓存已支持按站点导入、刷新导入、删除记录，以及从 Aura Profile 中清理该站点登录态。
5. Aura 浏览器 Profile 当前已支持打开目录、重置全部站点会话、清空整份 Profile。

本轮收口内容：

1. 系统前台 Chrome 自动化继续保留为备用模式，但新增了“运行时不可用时允许自动降级”开关，避免再次回到默认前台 Chrome 路径。
2. `chrome_*` 的审批入口已从通用审批区移出，收拢到浏览器页的“系统 Chrome 备用模式”模块。
3. 模型侧路由规则已明确为：普通网页任务走 `browser_*`；只有用户明确要求操作系统 Chrome，或 Aura 浏览器运行时不可用且允许降级时，才走 `chrome_*`。
4. 已导入站点管理现在会完整展示来源 Profile、首次导入时间、最近刷新时间和 Cookie 数量。
5. 设置页里原来的浏览器“状态占位”已移除，改为真实运行策略说明。
