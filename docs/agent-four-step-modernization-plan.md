...
| Step 4 | 已完成首轮落地 | 已补齐默认路径指标汇总、日志看板筛选、runId 聚合与回归验证入口；真实任务验收继续由你通过日志看板确认 |
...
> 当前状态：Step 4 已完成首轮工程落地。默认执行策略仍保持保守：简单任务走 Fast Path，复杂/graph/恢复任务走 Hybrid State Graph，普通任务继续保留 route-first 稳定执行内核；成熟化重点先落在“可观测、可回放、可验证”，避免为了切换架构牺牲现有执行能力。
...
1. `runAgent` 成功或失败都会输出 `agent.metrics.summary`，用于统一观察 `pathMode`、耗时、token、工具数、checkpoint 数、recovery 数和 graph 状态。
2. 日志看板新增范围筛选：全部、Agent、指标、排障；搜索与上下文聚合优先使用 `runId`，可以复盘一次完整 run。
3. 日志摘要增加架构关键字段：`architectureMode`、`pathMode`、`terminationReason`、`completionState`、`graphState`、`checkpointCount`、`recoveryCount`、token 与耗时。
4. Step 4 自动验证入口固化到 runtime log helper、Fast Path、Graph failure/recovery 相关测试。
5. Bridge 全量回归已收口到 `184/184` 通过；旧的 `invokeTool` 字符串断言已升级为结构化 `ToolResult` 断言，Provider retry delay 修正为固定阶梯重试。
...
`agent.validation.summary` 暂不由 runtime 自动伪造；它用于真实任务验收或测试脚本在场景结束后写入，避免把“未人工验证”的结果误标为已验证。

工具错误事件的 `errorInfo` 会保留底层 `code`，用于日志看板和回归测试直接定位错误类型，例如 `INVALID_APPLY_PATCH_INVOCATION`、`SHELL_FILE_MUTATION_BLOCKED`。
